...
static void sm100_m_grouped_fp4_gemm_masked_1d1d(const torch::Tensor& a, const torch::Tensor& sfa,
    ...
    // Zero-pad A/B along K (3D: [G, max_M, K] and [G, N, K]) if K is not aligned.
    // Avoids TMA OOB_FILL_NONE leaking stale memory for int32 reads.
    auto a_work = a;
    auto b_work = b;
    int k_work = k;
    if (k < aligned_k) {
        a_work = torch::zeros({num_groups, m, aligned_k}, a.options());
        a_work.slice(2, 0, k).copy_(a);
        b_work = torch::zeros({num_groups, n, aligned_k}, b.options());
        b_work.slice(2, 0, k).copy_(b);
        k_work = aligned_k;
    }

    // Pass `expected_m` as the useful per-group M so swap_ab heuristic can gate.
    const auto& config = get_best_fp4_config(GemmType::MGroupedMasked,
                                             expected_m, n, k, num_groups, major_a, major_b,
                                             d.scalar_type(), false,
                                             device_runtime->get_num_sms(),
                                             /*expected_m_per_group=*/expected_m);

    // Tensor descriptors. Masked layout: A/B/D all 3D with num_groups outer dim.
    const auto& tensor_map_a = make_tma_a_desc(major_a, a_work, m, k_work,
                                               SM100ArchSpec::get_ab_load_block_m(config.multicast_config, config.block_m),
                                               config.block_k,
                                               static_cast<int>(a_work.stride(get_non_contiguous_dim(major_a))), num_groups,
                                               config.smem_config.swizzle_a_mode);
    const auto& tensor_map_b = make_tma_b_desc(major_b, b_work, n, k_work,
                                               SM100ArchSpec::get_ab_load_block_n(config.multicast_config, config.block_n),
                                               config.block_k,
    ...
    const auto& tensor_map_d = make_tma_cd_desc(d, m, n,
                                                ...
                                                config.smem_config.swizzle_cd_mode);
    const int sf_block_k = 4;  // FP4-specific: SF block_k = 4 int32 units
    const auto& tensor_map_sfa = make_tma_sf_desc(cute::UMMA::Major::MN, sfa, m, k,
                                                  config.block_m, sf_block_k, num_groups, 0);
    const auto& tensor_map_sfb = make_tma_sf_desc(cute::UMMA::Major::MN, sfb, n, k,
                                                  config.block_n, sf_block_k, num_groups, 0);
